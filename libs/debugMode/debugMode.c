...
static uint64_t debug_ticks_to_us(uint64_t ticks) {
  ...
  return (ticks * 1000000ull + (state.tick_frequency >> 1)) /
         state.tick_frequency;
}

static void format_debug_text(uint32_t fps_x10, uint32_t rest_ms_x10,
                              uint32_t draw_buffer_ms_x10) {
  if (state.lcd_spi_baudrate_hz > 0u) {
    uint32_t spi_mhz_x10 = (state.lcd_spi_baudrate_hz + 50000u) / 100000u;
    snprintf(
        state.text, sizeof(state.text),
        "FPS: %lu.%lu\nREST: %lu.%lu  DRAW: %lu.%lu\nSPI: %lu.%lu MHz",
        (unsigned long)(fps_x10 / 10u), (unsigned long)(fps_x10 % 10u),
        (unsigned long)(rest_ms_x10 / 10u), (unsigned long)(rest_ms_x10 % 10u),
        (unsigned long)(draw_buffer_ms_x10 / 10u),
        (unsigned long)(draw_buffer_ms_x10 % 10u),
        (unsigned long)(spi_mhz_x10 / 10u), (unsigned long)(spi_mhz_x10 % 10u));
    return;
  }
...
static void update_debug_mode(uint64_t rest_ticks, uint64_t draw_buffer_ticks) {
  if (state.tick_frequency == 0u ||
      (rest_ticks == 0u && draw_buffer_ticks == 0u))
    return;
  ...
  if (state.window_us >= DEBUG_MODE_UPDATE_WINDOW_US) {
    uint32_t avg_frame_us =
        (uint32_t)((state.window_us + (state.window_frames >> 1)) /
                   state.window_frames);
    uint32_t avg_rest_us =
        (uint32_t)((state.rest_window_us + (state.window_frames >> 1)) /
                   state.window_frames);
    uint32_t avg_draw_buffer_us =
        (uint32_t)((state.draw_buffer_window_us + (state.window_frames >> 1)) /
                   state.window_frames);
    uint32_t fps_x10 = 0u;
    ...
    if (avg_frame_us > 0u) {
      fps_x10 = (uint32_t)((10000000ull + ((uint64_t)avg_frame_us >> 1)) /
                           (uint64_t)avg_frame_us);
      rest_ms_x10 = (uint32_t)(((uint64_t)avg_rest_us + 50ull) / 100ull);
      draw_buffer_ms_x10 =
          (uint32_t)(((uint64_t)avg_draw_buffer_us + 50ull) / 100ull);
    }
...
static void end_draw_buffer(void) {
  ...
  state.frame_draw_buffer_ticks +=
      draw_buffer_end_ticks - state.draw_buffer_begin_ticks;
  state.draw_buffer_active = 0u;
}
