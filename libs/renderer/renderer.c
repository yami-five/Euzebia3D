...
static inline int32_t shifted_delta_i32(int32_t a, int32_t b, int32_t shift,
                                        int32_t divisor) {
  if (divisor == 0)
    return 0;
...
static inline void set_active_texture_data(const uint16_t *texture,
...
static void prepare_texture_cache(Material *mat) {
...
#if EUZEBIA3D_TEXTURE_CACHE_ENABLED
  if (mat == NULL || mat->texture == NULL || mat->textureWidth <= 0 ||
      mat->textureHeight <= 0)
    return;

  if (mat->textureWidth > TEXTURE_CACHE_SIZE ||
      mat->textureHeight > TEXTURE_CACHE_SIZE)
    return;
  ...
  for (uint8_t i = 0; i < TEXTURE_CACHE_SLOTS; i++) {
    if (textureCache[i].valid && textureCache[i].source == mat->texture &&
        textureCache[i].width == mat->textureWidth &&
        textureCache[i].height == mat->textureHeight) {
      set_active_texture_data(textureCache[i].pixels, textureCache[i].width,
                              textureCache[i].height);
      return;
    }
  }
...
static uint8_t ensure_model_scratch_capacity(uint16_t verticesCounter,
                                             uint16_t vnCounter) {
  if (verticesCounter > modelScratchVertexCapacity) {
    ...
    int32_t *newVerticesModified = (int32_t *)realloc(
        modelScratchVerticesModified, sizeof(int32_t) * vertexTriplets);
    if (newVerticesModified == NULL)
      return 0;
    modelScratchVerticesModified = newVerticesModified;

    int32_t *newVerticesClip = (int32_t *)realloc(
        modelScratchVerticesClip, sizeof(int32_t) * vertexQuads);
  ...
  if (vnCounter > modelScratchNormalCapacity) {
    ...
    int32_t *newNormalsModified = (int32_t *)realloc(
        modelScratchNormalsModified, sizeof(int32_t) * normalTriplets);
...
static int32_t average_i32_pair(int32_t a, int32_t b) {
  return clamp_i64_to_i32(((int64_t)a + b) / 2);
}

static int32_t average_i32_triple(int32_t a, int32_t b, int32_t c) {
...
void transform(int32_t *vertices, uint16_t verticesCounter,
  ...
  if (vertices == NULL || transformInfo == NULL ||
      transformInfo->transformVector == NULL) {
...
static uint8_t triangle_outside_render_area(int32_t ax, int32_t ay, int32_t bx,
                                            int32_t by, int32_t cx,
                                            int32_t cy) {
  if (ax < 0 && bx < 0 && cx < 0)
    return 1;
  if (ay < 0 && by < 0 && cy < 0)
...
static uint8_t triangle_outside_raster_guard(int32_t ax, int32_t ay, int32_t bx,
                                             int32_t by, int32_t cx,
                                             int32_t cy) {
...
static uint8_t project_clip_point(const ClipVertex *in, Vector2 *out) {
  ...
  out->x = fixed_div(in->x, in->w) + render_width_half;
  out->y = fixed_div(in->y, in->w) + render_height_half;
  return 1;
}
...
static uint8_t clip_line_against_near_plane(ClipVertex *start,
  ...
  if (!startInside && !endInside)
    return 0;
  if (!startInside)
    *start = intersect_near_plane(start, end);
  else if (!endInside)
    *end = intersect_near_plane(start, end);
  return 1;
}
...
static uint8_t line_out_code(const Vector2 *point) {
  ...
  if (point->x < 0)
    code |= LINE_OUT_LEFT;
  else if (point->x >= render_width)
    code |= LINE_OUT_RIGHT;
  if (point->y < 0)
    code |= LINE_OUT_TOP;
  else if (point->y >= render_height)
...
static void build_material_span(uint16_t *dst, uint16_t length, Material *mat,
  ...
  if (mat->texture == NULL || mat->textureWidth <= 0 || mat->textureHeight <= 0)
    ...
    texture_span(dst, length, mat, lerp->U, lerp->dUdx, lerp->V, lerp->dVdx,
                 lerp->Z, lerp->dZdx);
...
inline int32_t calc_pixel_depth(int32_t Ba, int32_t Bb, int32_t Bc, int32_t z1,
  ...
  if (span_length > 0) {
    ...
    for (uint8_t dy = 0; dy < output_scale; dy++) {
      ...
      if (mat->transparent)
        ...
        _painter->draw_span(spanX0, spanY + dy, span_to_draw,
                            span_to_draw_length);
    }
  }
...
void tri(TriangleToRender *triangle, Material *mat, int32_t lightDistances[],
  ...
  int32_t dL01 =
      dy01
          ? ((lightDistances[1] - lightDistances[0]) << LIGHT_LERP_SHIFT) / dy01
          : 0;
  int32_t dL02 =
      dy02
          ? ((lightDistances[2] - lightDistances[0]) << LIGHT_LERP_SHIFT) / dy02
          : 0;
  int32_t dL12 =
      dy12
          ? ((lightDistances[2] - lightDistances[1]) << LIGHT_LERP_SHIFT) / dy12
          : 0;

  int32_t dU01 =
      shifted_delta_i32(triangle->uvA.x, triangle->uvB.x, UV_LERP_SHIFT, dy01);
  int32_t dV01 =
      shifted_delta_i32(triangle->uvA.y, triangle->uvB.y, UV_LERP_SHIFT, dy01);
  int32_t dU02 =
      shifted_delta_i32(triangle->uvA.x, triangle->uvC.x, UV_LERP_SHIFT, dy02);
  int32_t dV02 =
      shifted_delta_i32(triangle->uvA.y, triangle->uvC.y, UV_LERP_SHIFT, dy02);
  int32_t dU12 =
      shifted_delta_i32(triangle->uvB.x, triangle->uvC.x, UV_LERP_SHIFT, dy12);
  int32_t dV12 =
      shifted_delta_i32(triangle->uvB.y, triangle->uvC.y, UV_LERP_SHIFT, dy12);
...
static void draw_scaled_scene_pixel(int32_t x, int32_t y, uint16_t color) {
  ...
  int32_t firstX = x * output_scale;
  int32_t firstY = y * output_scale;
  for (uint8_t dy = 0; dy < output_scale; dy++) {
    int32_t drawY = firstY + dy;
    if (drawY < 0 || drawY >= BASE_HEIGHT)
      continue;
    for (uint8_t dx = 0; dx < output_scale; dx++) {
      int32_t drawX = firstX + dx;
      if (drawX < 0 || drawX >= BASE_WIDTH)
        continue;
      _painter->draw_pixel((uint16_t)drawX, (uint16_t)drawY, color);
    }
  }
}
...
static void draw_scene_line(const LineInScene *line) {
  ...
  _painter->draw_line(&start, &end, line->color);
}
...
void render_scene(Light *pLight) {
  ...
  for (uint16_t i = 0; i < sceneCounter; i++) {
    ...
    drawItems[i].index = i;
#if EUZEBIA3D_RENDERER_SCENE_SORT_ENABLED
    drawItems[i].depth = scene[i].pos.z;
#else
    drawItems[i].depth = 0;
#endif
  }
...
void add_model_to_scene(Mesh *mesh, Camera *camera, Light *pLight) {
  ...
  if (mesh->vertices == NULL || mesh->faces == NULL ||
      mesh->textureCoords == NULL || mesh->uv == NULL) {
    RENDERER_SET_DEBUG_STAGE(102);
    return;
  }

  if (verticesCounter == 0 || mesh->facesCounter == 0 ||
      textureCoordsCounter == 0) {
  ...
  memcpy(verticesModified, mesh->vertices,
         verticesCounter * 3 * sizeof(int32_t));
  ...
  for (uint32_t i = 0; i < faceIndexCount; i += 3u) {
    ...
    if (uvA >= textureCoordsCounter || uvB >= textureCoordsCounter ||
        uvC >= textureCoordsCounter) {
      RENDERER_SET_DEBUG_STAGE(152);
      continue;
    }

    if (mesh->normals[i] >= vnCounter || mesh->normals[i + 1] >= vnCounter ||
        mesh->normals[i + 2] >= vnCounter) {
    ...
    if (!read_normal_vector(&normalVectorA, normalsModified, vnCounter,
                            mesh->normals[i]) ||
        !read_normal_vector(&normalVectorB, normalsModified, vnCounter,
                            mesh->normals[i + 1]) ||
        !read_normal_vector(&normalVectorC, normalsModified, vnCounter,
                            mesh->normals[i + 2])) {
      lightDistances[0] = 0;
      lightDistances[1] = 0;
      lightDistances[2] = 0;
      ...
      lightDistances[0] =
          clamp_light_distance(dot_product(&normalVectorA, &lightDirectionA));
      lightDistances[1] =
          clamp_light_distance(dot_product(&normalVectorB, &lightDirectionB));
      lightDistances[2] =
    ...
    for (uint8_t t = 1; t + 1 < clippedCount; t++) {
      ...
      outPrimitive->pos =
          calculate_triangle_center(&outTriangle->TriangleOnScreen);
    }
  }
...
void add_point_to_scene(Point3D *point, Camera *camera) {
  ...
  primitive->payload.point.point = screenPoint;
  primitive->payload.point.color = point->color;
  primitive->pos.x = screenPoint.x;
  primitive->pos.y = screenPoint.y;
...
void add_line_to_scene(Line3D *line, Camera *camera) {
  ...
  primitive->payload.line.start = screenStart;
  primitive->payload.line.end = screenEnd;
  primitive->payload.line.color = line->color;
  primitive->pos.x = average_i32_pair(screenStart.x, screenEnd.x);
  primitive->pos.y = average_i32_pair(screenStart.y, screenEnd.y);
